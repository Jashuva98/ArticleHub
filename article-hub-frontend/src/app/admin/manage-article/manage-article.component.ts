...
export class ManageArticleComponent implements OnInit{
  ...
  tableData(){
    this.articleService.getAllArticle().subscribe((response:any)=>{
      ...
      if(error.error?.message){
        this.responseMesssge = error.error?.message;
      }else{
        this.responseMesssge = GlobalConstants.genericError;
      }
      this.snackbarServices.openSnackBar(this.responseMesssge);
    })
  }
  ...
  deleteProduct(id:any){
    this.articleService.deleteArticle(id).subscribe((response:any)=>{
      this.ngxService.stop();
      this.tableData();
      this.responseMesssge = response.message;
      this.snackbarServices.openSnackBar(this.responseMesssge);
    },(error:any)=>{
      this.ngxService.stop();
      console.log(error);
