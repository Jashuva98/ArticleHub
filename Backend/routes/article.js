...
router.get('/getPublishedArticle',(req,res)=>{
    ...
    connection.query(query,[article.title,article.content,article.categoryId,new Date(),article.status, article.id],(err,results)=>{
        if(!err){
            if(results.affectedRows == 0){
                return res.status(400).json({message:'article id does not found'})
            }
            return res.status(200).json({message:'article updated success'})
        }else{
            return res.status(500).json(err)
        }
    })
